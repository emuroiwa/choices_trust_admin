...
<?php include_once "Donations_Reportsmryinfo.php" ?>
<?php
...
$Donations_Report_summary = NULL; // Initialize page object first

class crDonations_Report_summary extends crDonations_Report {
	...
	// Page object name
	var $PageObjName = 'Donations_Report_summary';
	...
	function __construct() {
		...
		// Table object (Donations_Report)
		if (!isset($GLOBALS["Donations_Report"])) {
			$GLOBALS["Donations_Report"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["Donations_Report"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'Donations Report', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fDonations_Reportsummary";
	}
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_Donations_Report\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_Donations_Report',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fDonations_Reportsummary\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fDonations_Reportsummary\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fDonations_Reportsummary\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			if ($popupname == 'Donations_Report_name') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'Donations_Report_user') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'Donations_Report_ordertype') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'Donations_Report_transactiondate') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
				...
				while ($rswrk && !$rswrk->EOF) {
					...
					if (is_null($this->transactiondate->CurrentValue)) {
						...
						$this->transactiondate->ViewValue = ewr_FormatDateTime($this->transactiondate->CurrentValue, 5);
			...
			if ($popupname == 'Donations_Report_ordernumber') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'Donations_Report_status') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'Donations_Report_platform') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'Donations_Report_transactiontype') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
	...
	function SetupPopup() {
		...
		if (is_array(@$_SESSION["sel_Donations_Report_name"])) {
			$this->LoadSelectionFromSession('name');
		} elseif (@$_SESSION["sel_Donations_Report_name"] == EWR_INIT_VALUE) { // Select all
			$this->name->SelectionList = "";
		}

		// Get user selected values
		if (is_array(@$_SESSION["sel_Donations_Report_user"])) {
			$this->LoadSelectionFromSession('user');
		} elseif (@$_SESSION["sel_Donations_Report_user"] == EWR_INIT_VALUE) { // Select all
			$this->user->SelectionList = "";
		}

		// Get ordertype selected values
		if (is_array(@$_SESSION["sel_Donations_Report_ordertype"])) {
			$this->LoadSelectionFromSession('ordertype');
		} elseif (@$_SESSION["sel_Donations_Report_ordertype"] == EWR_INIT_VALUE) { // Select all
			$this->ordertype->SelectionList = "";
		}

		// Get transactiondate selected values
		if (is_array(@$_SESSION["sel_Donations_Report_transactiondate"])) {
			$this->LoadSelectionFromSession('transactiondate');
		} elseif (@$_SESSION["sel_Donations_Report_transactiondate"] == EWR_INIT_VALUE) { // Select all
			$this->transactiondate->SelectionList = "";
		}

		// Get ordernumber selected values
		if (is_array(@$_SESSION["sel_Donations_Report_ordernumber"])) {
			$this->LoadSelectionFromSession('ordernumber');
		} elseif (@$_SESSION["sel_Donations_Report_ordernumber"] == EWR_INIT_VALUE) { // Select all
			$this->ordernumber->SelectionList = "";
		}

		// Get status selected values
		if (is_array(@$_SESSION["sel_Donations_Report_status"])) {
			$this->LoadSelectionFromSession('status');
		} elseif (@$_SESSION["sel_Donations_Report_status"] == EWR_INIT_VALUE) { // Select all
			$this->status->SelectionList = "";
		}

		// Get platform selected values
		if (is_array(@$_SESSION["sel_Donations_Report_platform"])) {
			$this->LoadSelectionFromSession('platform');
		} elseif (@$_SESSION["sel_Donations_Report_platform"] == EWR_INIT_VALUE) { // Select all
			$this->platform->SelectionList = "";
		}

		// Get transactiontype selected values
		if (is_array(@$_SESSION["sel_Donations_Report_transactiontype"])) {
			$this->LoadSelectionFromSession('transactiontype');
		} elseif (@$_SESSION["sel_Donations_Report_transactiontype"] == EWR_INIT_VALUE) { // Select all
			$this->transactiontype->SelectionList = "";
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			$this->transactiondate->ViewValue = ewr_FormatDateTime($this->transactiondate->ViewValue, 5);
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_Donations_Report_$parm"] = "";
		$_SESSION["rf_Donations_Report_$parm"] = "";
		$_SESSION["rt_Donations_Report_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->fields($parm);
		$fld->SelectionList = @$_SESSION["sel_Donations_Report_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_Donations_Report_$parm"];
		$fld->RangeTo = @$_SESSION["rt_Donations_Report_$parm"];
	}
	...
	function RestoreFilterList() {
		...
		if (array_key_exists("sel_name", $filter)) {
			...
			$_SESSION["sel_Donations_Report_name"] = $sWrk;
			$bRestoreFilter = TRUE;
		}
		...
		if (array_key_exists("sel_user", $filter)) {
			...
			$_SESSION["sel_Donations_Report_user"] = $sWrk;
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
		}
		...
		if (array_key_exists("sel_ordertype", $filter)) {
			...
			$_SESSION["sel_Donations_Report_ordertype"] = $sWrk;
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
		}
		...
		if (array_key_exists("sel_transactiondate", $filter)) {
			...
			$_SESSION["sel_Donations_Report_transactiondate"] = $sWrk;
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
		}
		...
		if (array_key_exists("sel_ordernumber", $filter)) {
			...
			$_SESSION["sel_Donations_Report_ordernumber"] = $sWrk;
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
		}
		...
		if (array_key_exists("sel_status", $filter)) {
			...
			$_SESSION["sel_Donations_Report_status"] = $sWrk;
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
		}
		...
		if (array_key_exists("sel_platform", $filter)) {
			...
			$_SESSION["sel_Donations_Report_platform"] = $sWrk;
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
		}
		...
		if (array_key_exists("sel_transactiontype", $filter)) {
			...
			$_SESSION["sel_Donations_Report_transactiontype"] = $sWrk;
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
		}
...
// Create page object
if (!isset($Donations_Report_summary)) $Donations_Report_summary = new crDonations_Report_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$Donations_Report_summary;
...
// Create page object
var Donations_Report_summary = new ewr_Page("Donations_Report_summary");

// Page properties
Donations_Report_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = Donations_Report_summary.PageID;

// Extend page with Chart_Rendering function
Donations_Report_summary.Chart_Rendering = 
...
Donations_Report_summary.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var CurrentForm = fDonations_Reportsummary = new ewr_Form("fDonations_Reportsummary");
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fDonations_Reportsummary" id="fDonations_Reportsummary" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
</form>
<script type="text/javascript">
fDonations_Reportsummary.Init();
fDonations_Reportsummary.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "Donations_Reportsmrypager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<!-- Report grid (begin) -->
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="name"><div class="Donations_Report_name"><span class="ewTableHeaderCaption"><?php echo $Page->name->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="name">
<?php if ($Page->SortUrl($Page->name) == "") { ?>
		<div class="ewTableHeaderBtn Donations_Report_name">
			<span class="ewTableHeaderCaption"><?php echo $Page->name->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Donations_Report_name', false, '<?php echo $Page->name->RangeFrom; ?>', '<?php echo $Page->name->RangeTo; ?>');" id="x_name<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Donations_Report_name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->name) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->name->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="user"><div class="Donations_Report_user"><span class="ewTableHeaderCaption"><?php echo $Page->user->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="user">
<?php if ($Page->SortUrl($Page->user) == "") { ?>
		<div class="ewTableHeaderBtn Donations_Report_user">
			<span class="ewTableHeaderCaption"><?php echo $Page->user->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Donations_Report_user', false, '<?php echo $Page->user->RangeFrom; ?>', '<?php echo $Page->user->RangeTo; ?>');" id="x_user<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Donations_Report_user" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->user) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->user->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="ordertype"><div class="Donations_Report_ordertype"><span class="ewTableHeaderCaption"><?php echo $Page->ordertype->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="ordertype">
<?php if ($Page->SortUrl($Page->ordertype) == "") { ?>
		<div class="ewTableHeaderBtn Donations_Report_ordertype">
			<span class="ewTableHeaderCaption"><?php echo $Page->ordertype->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Donations_Report_ordertype', false, '<?php echo $Page->ordertype->RangeFrom; ?>', '<?php echo $Page->ordertype->RangeTo; ?>');" id="x_ordertype<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Donations_Report_ordertype" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->ordertype) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->ordertype->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="transactiondate"><div class="Donations_Report_transactiondate"><span class="ewTableHeaderCaption"><?php echo $Page->transactiondate->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="transactiondate">
<?php if ($Page->SortUrl($Page->transactiondate) == "") { ?>
		<div class="ewTableHeaderBtn Donations_Report_transactiondate">
			<span class="ewTableHeaderCaption"><?php echo $Page->transactiondate->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Donations_Report_transactiondate', false, '<?php echo $Page->transactiondate->RangeFrom; ?>', '<?php echo $Page->transactiondate->RangeTo; ?>');" id="x_transactiondate<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Donations_Report_transactiondate" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->transactiondate) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->transactiondate->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="ordernumber"><div class="Donations_Report_ordernumber"><span class="ewTableHeaderCaption"><?php echo $Page->ordernumber->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="ordernumber">
<?php if ($Page->SortUrl($Page->ordernumber) == "") { ?>
		<div class="ewTableHeaderBtn Donations_Report_ordernumber">
			<span class="ewTableHeaderCaption"><?php echo $Page->ordernumber->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Donations_Report_ordernumber', false, '<?php echo $Page->ordernumber->RangeFrom; ?>', '<?php echo $Page->ordernumber->RangeTo; ?>');" id="x_ordernumber<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Donations_Report_ordernumber" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->ordernumber) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->ordernumber->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="status"><div class="Donations_Report_status"><span class="ewTableHeaderCaption"><?php echo $Page->status->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="status">
<?php if ($Page->SortUrl($Page->status) == "") { ?>
		<div class="ewTableHeaderBtn Donations_Report_status">
			<span class="ewTableHeaderCaption"><?php echo $Page->status->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Donations_Report_status', false, '<?php echo $Page->status->RangeFrom; ?>', '<?php echo $Page->status->RangeTo; ?>');" id="x_status<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Donations_Report_status" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->status) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->status->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="platform"><div class="Donations_Report_platform"><span class="ewTableHeaderCaption"><?php echo $Page->platform->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="platform">
<?php if ($Page->SortUrl($Page->platform) == "") { ?>
		<div class="ewTableHeaderBtn Donations_Report_platform">
			<span class="ewTableHeaderCaption"><?php echo $Page->platform->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Donations_Report_platform', false, '<?php echo $Page->platform->RangeFrom; ?>', '<?php echo $Page->platform->RangeTo; ?>');" id="x_platform<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Donations_Report_platform" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->platform) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->platform->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="transactiontype"><div class="Donations_Report_transactiontype"><span class="ewTableHeaderCaption"><?php echo $Page->transactiontype->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="transactiontype">
<?php if ($Page->SortUrl($Page->transactiontype) == "") { ?>
		<div class="ewTableHeaderBtn Donations_Report_transactiontype">
			<span class="ewTableHeaderCaption"><?php echo $Page->transactiontype->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'Donations_Report_transactiontype', false, '<?php echo $Page->transactiontype->RangeFrom; ?>', '<?php echo $Page->transactiontype->RangeTo; ?>');" id="x_transactiontype<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer Donations_Report_transactiontype" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->transactiontype) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->transactiontype->FldCaption() ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_Donations_Report_name"<?php echo $Page->name->ViewAttributes() ?>><?php echo $Page->name->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->user->Visible) { ?>
		<td data-field="user"<?php echo $Page->user->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_Donations_Report_user"<?php echo $Page->user->ViewAttributes() ?>><?php echo $Page->user->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->ordertype->Visible) { ?>
		<td data-field="ordertype"<?php echo $Page->ordertype->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_Donations_Report_ordertype"<?php echo $Page->ordertype->ViewAttributes() ?>><?php echo $Page->ordertype->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->transactiondate->Visible) { ?>
		<td data-field="transactiondate"<?php echo $Page->transactiondate->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_Donations_Report_transactiondate"<?php echo $Page->transactiondate->ViewAttributes() ?>><?php echo $Page->transactiondate->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->ordernumber->Visible) { ?>
		<td data-field="ordernumber"<?php echo $Page->ordernumber->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_Donations_Report_ordernumber"<?php echo $Page->ordernumber->ViewAttributes() ?>><?php echo $Page->ordernumber->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->status->Visible) { ?>
		<td data-field="status"<?php echo $Page->status->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_Donations_Report_status"<?php echo $Page->status->ViewAttributes() ?>><?php echo $Page->status->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->platform->Visible) { ?>
		<td data-field="platform"<?php echo $Page->platform->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_Donations_Report_platform"<?php echo $Page->platform->ViewAttributes() ?>><?php echo $Page->platform->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->transactiontype->Visible) { ?>
		<td data-field="transactiontype"<?php echo $Page->transactiontype->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->RecCount ?>_Donations_Report_transactiontype"<?php echo $Page->transactiontype->ViewAttributes() ?>><?php echo $Page->transactiontype->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<span data-class="tpts_Donations_Report_ordertype"<?php echo $Page->ordertype->ViewAttributes() ?>><?php echo $Page->ordertype->SumViewValue ?></span></td>
<?php } ?>
<?php if ($Page->transactiondate->Visible) { ?>
